classdef test_shift <  TestCaseWithSave
    % Test class to test shift methods
    ...
    methods
        function obj=test_shift (name)
            obj@TestCaseWithSave(name);
            obj.save()
            
            % Make 1D dataset
            nx = 10;
            x = linspace_rand (1, 10, nx, 0.2, 'seed', 0);
            obj.w1p = IX_dataset_1d (x, rand(nx,1), rand(size(x)), ...
                'A title', 'x-axis', 'signal axis', true);
            
            obj.w1h = IX_dataset_1d (x, rand(size(x)), rand(size(x)), ...
                'A title', 'x-axis', 'signal axis', true);
        ...
        function test_1D(obj)
            ...
            w = func_eval (w, @linear_bg, [5,1.2]);
            wd = deriv (w);
        end
        
        function test_1D_error(obj)
            testfun = @()shift (obj.w1, [5,2]);
            assertExceptionThrown(testfun, 'HERBERT:shift_:invalid_argument');
        end
        ...
        function test_2D_error(obj)
            testfun = @()shift (obj.w2, 5);
            assertExceptionThrown(testfun, 'HERBERT:shift_:invalid_argument');
        end
        
        function test_2D_error2(obj)
            testfun = @()shift (obj.w2, []);
            assertExceptionThrown(testfun, 'HERBERT:shift_:invalid_argument');
        end
        
        function test_2D_error_x(obj)
            testfun = @()shift_x (obj.w2, [5,3]);
            assertExceptionThrown(testfun, 'HERBERT:shift_:invalid_argument');
        end
        ...
        function test_3D_error(obj)
            testfun = @()shift (obj.w3, 5);
            assertExceptionThrown(testfun, 'HERBERT:shift_:invalid_argument');
        end
        
        function test_3D_error_y(obj)
            testfun = @()shift_y (obj.w3, [5,-3,99]);
            assertExceptionThrown(testfun, 'HERBERT:shift_:invalid_argument');
        end
        ...
        function test_2D_array(obj)
            ...
            ws = shift (w, [5,-3]);
            % Reference answer
            ws_ref = [shift(obj.w2, [5,-3]), shift(obj.w2b, [5,-3])];
            % Test
            assertEqual (ws_ref, ws) 
        end
    end
