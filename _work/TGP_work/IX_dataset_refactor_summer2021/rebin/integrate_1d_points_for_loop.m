...
%
%**************************************************************************
% T.G.Perring 25 Aug 2021:
% -----------------------
% Original integrate_1d_points algorithm in herbert used for several years
% (location: ..\Herbert\herbert_core\utilities\maths but with a simple for
% loop summing over the bins instead of the original vectorised algorithm
% using repmat. Hopefully to refactor to perform true error bar calculation
% and also for loops found to be very fast with the rebin algorithm.
%
% The name of the function is historic; the name of the file indicates the
...
if mu<ml
    ...
    elseif mu==ml+1	% one complete interval
        ...
        % Refactored code. I have kept the old code to allow easy reversion
        % for testing the Matlab JIT compiler from release to release.
        % Extraordinarily, the shocking-looking for loops are fine because
        % they get JITted away - but NOT if you use the profiler. tic-toc 
...
%             + sum((e(ml+1:mu-1).*xwrk).^2, iax);
        ...
        % New code - verified works to within O(eps)
        for i=ml:mu-1
