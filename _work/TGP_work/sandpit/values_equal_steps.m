...
if x1 < x2
    ...
    if strcmp(origin,'x1')
        ...
        np = floor((x2-x1)/del - tol);
        if high_closed && (x1+del*np) >= x2 - tol
            np = np - 1;    % catch case of tol==0 &/or rounding error
        end
        if nargout==2
    ...
    elseif strcmp(origin,'x2')
        ...
        np = floor((x2-x1)/del - tol);
        if low_closed && (x2-del*np) <= x1 + tol
            np = np - 1;    % catch case of tol==0 &/or rounding error
        end
        if nargout==2
            if low_closed && high_closed
    ...
    elseif strcmp(origin,'c0')
        ...
        if low_closed && (del*(2*nlo+1))/2 <= x1 + tol
            nlo = nlo + 1;    % catch case of tol==0 &/or rounding error
        end
        if high_closed && (del*(2*nhi+1))/2 >= x2 - tol
    ...
    elseif strcmp(origin,'v0')
        ...
        if low_closed && del*nlo <= x1 + tol
            nlo = nlo + 1;    % catch case of tol==0 &/or rounding error
        end
        if high_closed && del*nhi >= x2 - tol
            nhi = nhi - 1;    % catch case of tol==0 &/or rounding error
        end
