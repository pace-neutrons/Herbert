function [sout, eout] = rebin_hist_trueErrors_Nx(x, s, e, idim, xout)
...
%
% *** Method permutes the integration axis to the first of two dimensions
%
% Input:
% ------
...
s = permute(s,[2,3,1]);
e = permute(e,[2,3,1]);
s = reshape(s,[mx, prod(sz)/mx]);
e = reshape(e,[mx, prod(sz)/mx]);
...
sout = reshape(sout, [nx, prod(sz(idim+1:end)), prod(sz(1:idim-1))]);
eout = reshape(eout, [nx, prod(sz(idim+1:end)), prod(sz(1:idim-1))]);
sout = permute(sout, [3,1,2]);
eout = permute(eout, [3,1,2]);
