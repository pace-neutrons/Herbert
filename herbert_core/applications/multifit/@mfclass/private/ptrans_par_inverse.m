...
% $Revision:: 840 ($Date:: 2020-02-10 16:05:56 +0000 (Mon, 10 Feb 2020) $)
    ...
    if ~isstruct(S)
        error('Herbert:mfclass:badargs', 'Input argument is not a structure')
    end
...
% Check that the parameters generated fromt the free parameters are consistent with input
    ...
    if ~equal_to_relerr(pp_in,pp,tol,min_denominator)
        error('Herbert:mfclass:badbind', 'The fixed and/or binding ratios of input argument parameters are inconsistent with those of the object');
    end
end
        ...
        if isfield(S,nam)
            if isnumeric(S.(nam))
                if numel(np) == 1 && numel(S.(nam)) == nptot
                    ...
                    error('Herbert:mfclass:badargs', ['Input argument ',str,'ground parameter array length is inconsistent with object']);
                end
            elseif iscell(S.(nam)) && all(cellfun(@isnumeric,S.(nam)))
                if numel(S.(nam)) == numel(np) && all(cellfun(@numel,S.(nam)) == np)
                    ...
                    error('Herbert:mfclass:badargs', ['Input argument ',str,'ground parameter cell array or element lengths are inconsistent with object'])
                end
            else
                error('Herbert:mfclass:badargs', ['Input argument ',str,'ground parameter field is not a single numeric array or a cell array of numeric arrays']);
            end
        else
            error('Herbert:mfclass:badargs', ['Input argument does not contain any ',str,'ground parameter values']);
        end
    else
        if isfield(S,nam)
