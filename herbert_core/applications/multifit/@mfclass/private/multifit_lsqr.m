...
if exist('perform_fit', 'var') && ~perform_fit
    ...
    if is_parallel && ~all(xye)
        ...
        if any(cellfun(@(x)(startsWith(functions(x).function, 'tobyfit')), func))
            loop_data = split_data(w, xye, [], [], nWorkers, arrayfun(@(x)(x.plist{3}), pin, 'UniformOutput', false));
        else
    ...
    for iter=1:niter
        ...
        jac=multifit_dfdpf(w,xye,func,bfunc,pin,bpin,...
                           f_pass_caller_info,bf_pass_caller_info,p_best,p_info,f_best,dp,S,Store,listing,jd,common_data,loop_data);
        nrm=zeros(npfree,1);
        for k=1:npfree
            jac(:,k)=wt.*jac(:,k);
            nrm(k)=jac(:,k)'*jac(:,k);
...
function jac=multifit_dfdpf(w,xye,func,bfunc,pin,bpin,...
                            f_pass_caller_info,bf_pass_caller_info,p,p_info,f,dp,S,Store,listing,jd,common_data,loop_data)
...
function [loop_data, merge_data] = split_data(w, xye, S, Store, nWorkers, tobyfit)
% Split up sqws and divvy xyes in w
    ...
    for i=1:numel(w)
        if xye(i)
            ...
            [data, merge_data{i}] = split_sqw(w{i}, 'nWorkers', nWorkers);
            for j=1:nWorkers
    ...
    for i=1:nWorkers
        ...
        for j=1:numel(w)
            loop_data{i}.nelem = [loop_data{i}.nelem, merge_data{j}(i).nelem];
            loop_data{i}.nomerge{j} = merge_data{j}(i).nomerge;
        end
    end
        ...
        for i=1:nWorkers
            ...
            for k = 1:numel(tobyfit)
                for j = 1:numel(tobyfit{k}.kf)
                    ...
                    n = numel(tobyfit{k}.x2{j});
                    nPer = repmat(floor(n / nWorkers), nWorkers, 1);
                    nPer(1:mod(n, nWorkers)) = nPer(1:mod(n, nWorkers)) + 1;
                    points = [0; cumsum(nPer)];

                    loop_data{i}.tobyfit_data{k}.x2{j}     = tobyfit{k}.x2{j}(points(i)+1:points(i+1));
                    loop_data{i}.tobyfit_data{k}.detdcn{j} = tobyfit{k}.detdcn{j}(:,points(i)+1:points(i+1));
                    loop_data{i}.tobyfit_data{k}.f_mat{j}  = tobyfit{k}.f_mat{j}(:,:,points(i)+1:points(i+1));
                    loop_data{i}.tobyfit_data{k}.d_mat{j}  = tobyfit{k}.d_mat{j}(:,:,points(i)+1:points(i+1));
                end
            end
        end
    end
