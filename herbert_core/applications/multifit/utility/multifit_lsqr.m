...
% $$$               [outputs, ~, loop_data{1}.S, loop_data{1}.Store] = multifit_lsqr_func_eval( ...
% $$$                   loop_data{1}.w, ...
% $$$                   loop_data{1}.xye, ...
% $$$                   common_data.func, ...
% $$$                   common_data.bfunc, ...
% $$$                   common_data.pin, ...
% $$$                   common_data.bpin, ...
% $$$                   common_data.f_pass_caller_info, ...
% $$$                   common_data.bf_pass_caller_info, ...
% $$$                   common_data.p, ...
% $$$                   common_data.p_info, ...
% $$$                   true, ...
% $$$                   loop_data{1}.S, ...
...
% $$$                   common_data.listing);
                ...
                [outputs, n_failed, task_ids, jd] = jd.start_job('mfparallel', common_data, loop_data, true, nWorkers);
                ...
                outputs{2}
                outputs{3}
                loop_data{:}.S = outputs{:}.S;
                loop_data{:}.Store = outputs{:}.Store;
                f = outputs.f;
                resid=wt.*(yval-f);
                c=resid'*resid;
                if c<c_best || c==0
