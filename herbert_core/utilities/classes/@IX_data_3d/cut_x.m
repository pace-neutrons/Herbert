function obj_out = cut_x (obj, varargin)
% Make a cut from an IX_dataset_3d object or array of IX_dataset_3d objects along the x-axis
%
%   >> obj_out = cut_x (obj, descr)
%   >> obj_out = cut_x (obj, wref)           % reference object to provide output bins
%
%   >> obj_out = cut_x (..., 'int')          % change averaging method for point data
%   
% Input:
% ------
...
% EXAMPLES
%   >> obj_out = cut_x (obj, [])
%   >> obj_out = cut_x (obj, [-Inf,Inf])    % equivalent to above
%   >> obj_out = cut_x (obj, 10)
%   >> obj_out = cut_x (obj, [2000,3000])
%   >> obj_out = cut_x (obj, [2000,Inf])
%   >> obj_out = cut_x (obj, [2000,3000,4000,5000,6000])
%
...
obj_out = cut_ (obj, 1, array_is_descriptor, varargin{:});
