function [xbounds,any_lim_inf,is_descriptor,any_dx_zero]=...
    rebin_boundaries_description_parse_(nax,opt,varargin)
% Check rebin descriptor has valid format, and returns 
% in standard form [x1,dx1,x2,dx2,x3,...]
%
%   >> [ok,xbounds,any_lim_inf,is_descriptor,any_dx_zero,mess]=...
%   rebin_boundaries_description_parse(nax,opt)    % sets output descriptors to [] for all nax axes
%
%   >> [...]=rebin_boundaries_description_parse(nax,opt,arg1,arg2,...)    % general case (see below)
%
% Input:
% ------
...
%
%   descriptor_opts
%               Type check options: structure with fields
...
%
%   p1, p2,...  Sequence of binning descriptions, one per axis
%
%   One rebin axis (i.e. nax=1 only):
%   ---------------------------------
%     [] or ''              Single bin over full range of data i.e. equivalent to
%                           [-Inf,Inf] (empty_is_one_bin==true)
%                      *OR* 
%                           Leave bins as they are (empty_is_one_bin==false)
%
%     0                     Leave bins as they are i.e. treated as [-Inf,0,Inf]
%
%     dx                    Equally spaced bins width dx centred on x=0 and
%                           over full range of data i.e. equivalent to [-Inf,dx,Inf]
%                           (must have dx>0)
%
%     x1,x2                 Single bin (range_is_one_bin==true)
%                      *OR* 
%                           Keep original bins in the range xlo to xhi i.e.
...
%  - If values_are_boundaries==false:
...
% Output:
% -------
%   xbounds         Cell array of rebin vectors {xbins1, xbins2,...}; [] if ok==false.
%                   The form of each vector is
