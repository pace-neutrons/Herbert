function [wout,ok_rebin,mess] = rebin_object_single_(win,iax,xbounds,...
    true_values, is_descriptor, integrate_data, point_integration)
...
ax=axis(win,iax(1));
[wout_x{1},wout_s,wout_e,ok_rebin,mess] = rebin_one_axis(win,ndim,iax(1),...
    ax.values,win.signal_,win.error_,ax.distribution,...
    xbounds{1},true_values(1),is_descriptor(1),...
    integrate_data,point_integration(1));
if ~ok_rebin, wout=IX_dataset_nd(ndim); return, end

for i=2:nrebin
    ax=axis(win,iax(i));
    [wout_x{i},wout_s,wout_e,ok_rebin,mess] = rebin_one_axis(win,ndim,iax(i),...
        ax.values,wout_s,wout_e,ax.distribution,...
        xbounds{i},true_values(i),is_descriptor(i),...
        integrate_data,point_integration(i));
    if ~ok_rebin, wout=IX_dataset_nd(ndim); return, end
end

if ~integrate_data
    % distribution is same as input data
    wout=set_simple_xsigerr(win,iax,wout_x,wout_s,wout_e);         
else
    % reset distribution to false along integration axes
    wout=set_simple_xsigerr(win,iax,wout_x,wout_s,wout_e,false);   
end
